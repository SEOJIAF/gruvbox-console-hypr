#!/usr/bin/env bash
# gruvbox-console-hypr installer for Fedora and Arch Linux
...
SUDO_KEEPALIVE_PID=''
TRACE=0
DISTRO=''           # fedora | arch | unknown
AUR_HELPER=''       # yay | paru | '' (auto-detected on Arch)

log()  { printf '\u001B[1;33m==>\u001B[0m %s
' "$*" | tee -a "$LOG_FILE" >&2; }
ok()   { printf '\u001B[1;32m OK\u001B[0m %s
' "$*" | tee -a "$LOG_FILE" >&2; }
warn() { printf '\u001B[1;31mWARN\u001B[0m %s
' "$*" | tee -a "$LOG_FILE" >&2; }
die()  { warn "$*"; exit 1; }

# Run a command, streaming its real output live to the terminal (so slow
# steps like "pacman -S" show actual progress instead of looking frozen)
...
Supported distributions:
  - Fedora: installs from official repos + lionheartp/Hyprland COPR
  - Arch Linux: installs from official repos + AUR (noctalia-shell)

On Arch Linux, an AUR helper (yay or paru) is required for noctalia-shell.
If neither is found, the script will offer to install yay.

This installer maintains KDE Plasma as a separate login-session option and
does not store, request, or embed sudo passwords — sudo prompts for its own
password interactively as needed.
EOF
}
...
# ---------------------------------------------------------------------------
# Package installation: Fedora
# ---------------------------------------------------------------------------

install_packages_fedora() {
    have dnf || die "dnf is required on Fedora."
...
install_aur_helper() {
    ...
    run_step sudo pacman -S --needed --noconfirm base-devel git
    
    local tmpdir
    tmpdir="$(mktemp -d)"
    
    log "Cloning yay from AUR"
    run_step git clone https://aur.archlinux.org/yay.git "$tmpdir/yay"
    
    log "Building and installing yay"
    (cd "$tmpdir/yay" && run_step makepkg -si --noconfirm)
    ...
    if have yay; then
        ...
        die "Failed to install yay. Please install an AUR helper manually."
    fi
}

install_packages_arch() {
    have pacman || die "pacman is required on Arch Linux."

    # Sync package databases
    log "Syncing package databases"
    run_step sudo pacman -Sy

    # Install packages from official repos
    log "Installing Hyprland, portal integration, theming tools, and helpers from official repos"
    run_step sudo pacman -S --needed --noconfirm \
        hyprland \
        xdg-desktop-portal-hyprland \
        qt5-wayland \
        qt6ct \
    ...
    if [[ -z $AUR_HELPER ]]; then
        ...
        if (( ASSUME_YES )) || [[ ! -t 0 ]]; then
            ...
            printf '
'
            read -r -p "Install yay as AUR helper? [Y/n]: " answer
            if [[ ${answer:-Y} =~ ^[Yy]?$ ]]; then
                install_aur_helper
            else
                warn "Skipping AUR packages. noctalia-shell will not be installed."
                warn "Install it manually later with: yay -S noctalia-shell"
    ...
    log "Installing noctalia-shell from AUR using $AUR_HELPER"
    run_step "$AUR_HELPER" -S --needed --noconfirm noctalia-shell
...
install_packages() {
    case $DISTRO in
        ...
        arch)
            ...
            ;;
        *)
            die "Unsupported distribution. This installer supports Fedora and Arch Linux."
            ;;
    esac
}

# ---------------------------------------------------------------------------
# Hyprland configuration tweaks (distro-agnostic)
# ---------------------------------------------------------------------------
...
# ---------------------------------------------------------------------------
# Config linking (distro-agnostic)
# ---------------------------------------------------------------------------
...
ensure_whiptail() {
    ...
    case $DISTRO in
        fedora)
            ...
            ;;
        arch)
            if have pacman && sudo -v 2>/dev/null; then
                run_step sudo pacman -S --needed --noconfirm newt && have whiptail && return 0
            fi
            ;;
    esac
    return 1
...
"This installs Hyprland + Noctalia as a SECOND login-session option, alongside your existing KDE Plasma session.
...
"
    (( DO_PACKAGES )) && summary+="  - Install packages ($DISTRO)
"
...
# "pacman -S" alone can take minutes, and a gauge with no new progress to
...
run_install() {
    ...
    if (( DO_PACKAGES )); then
        ...
        SUDO_KEEPALIVE_PID=$!
        announce_step "installing packages — this can take a few minutes, real output follows below"
        install_packages
    fi
    if (( DO_SESSION_TWEAKS )); then
...
require_repo_layout
detect_distro
[[ $DISTRO != unknown ]] || die "Unsupported distribution. This installer supports Fedora and Arch Linux."
...
case $UI_MODE in
    gui)
        have zenity || { warn 'Zenity is not installed yet; installing it first.'
            case $DISTRO in
                fedora) sudo dnf install -y zenity || die "Could not install zenity" ;;
                arch) sudo pacman -S --needed --noconfirm zenity || die "Could not install zenity" ;;
            esac
        }
        gui_choices
        run_install
